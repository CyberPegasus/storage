...
# Reasonable settings, for all-dataset settings, please change 'hRng': (55, np.inf) to 'hRng': (-np.inf, np.inf)
TEST_SET = {'hRng': (55, np.inf), 'xRng':(5, 635), 'yRng':(5, 507), 'wRng':(-np.inf, np.inf)}
def check_xywh(xmin:int,ymin:int,w:int,h:int,LOAD_SET:dict=TEST_SET):
...
test_split = 'datasets/kaist/split/test.txt'
...
json_save_name = write_path + 'instances_test.json' if one_class_mode else write_path + 'instances_test_multilabel.json'
...
print("test sample num: ", len(sample_list))
...
for sample in pbar:
    ...
    # img read
    ir_img_path = modality['ir']+'/'+sample.split('/')[0]+'_'+sample.split('/')[1]+'_'+_img_name+'_'+modality['ir']+'.png'
    rgb_img_path = modality['rgb']+'/'+sample.split('/')[0]+'_'+sample.split('/')[1]+'_'+_img_name+'_'+modality['rgb']+'.png'
    assert os.path.exists(imgs_path+ir_img_path) and os.path.exists(imgs_path+rgb_img_path), f'{imgs_path+ir_img_path} or {imgs_path+rgb_img_path} not exists'
    ...
    assert len(rgb_txt)==len(ir_txt), f'rgb len:{len(rgb_txt)} != ir len:{len(ir_txt)}'
    if len(rgb_txt)==0:
        pass
    else:
        for index in range(len(rgb_txt)):
            ...
            xmin = (int(rgb_line[1]) + int(ir_line[1]))//2
            xmax = (int(rgb_line[1]) + int(rgb_line[3]) + int(ir_line[1]) + int(ir_line[3]))//2
            ymin = (int(rgb_line[2]) + int(ir_line[2]))//2
            ymax = (int(rgb_line[2]) + int(rgb_line[4]) + int(ir_line[2]) + int(ir_line[4]))//2
            
            ignore = 0
            try:
                class_id = class_id_dict[rgb_line[0]]
                if rgb_line[0] == 'person?':
                    ignore=1
            except:
                class_id = 0
            ...
            b_h = abs(ymax-ymin)
            b_area = b_w*b_h
            if not check_xywh(xmin,ymin,b_w,b_h):
                ignore = 1
            annotation = {
                'image_id': count,
